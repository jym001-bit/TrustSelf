...
- 独立终端中，BottomStatusBar 是 JLine `Status` 托管的底部 dock；JetBrains/JediTerm 和 VS Code/Cursor 内嵌终端不启用滚动保留区，改由 `SlashMenuLineReader.post` 在输入框下方绘制相同状态，输入提交时与编辑行一起擦除。两条路径都不手写 `\n`、`moveUp`、`CLEAR_TO_EOS` 或绝对光标行号；状态上层展示 YOLO/HITL 与 MCP/Skill 摘要，下层展示 model、phase、ctx、token、cost、elapsed 与 cwd。`ctx` 只表示当前仍会带入下一轮请求的上下文估算，`in/out/cache` 表示最近任务调用统计。
