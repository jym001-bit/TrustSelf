...
终端布局：状态与 thinking activity 统一由 JLine 底部区域管理；正文和输入行顺序显示，不主动跳到底部。Windows 正文与状态区共用 terminal writer，避免混用输出通道造成残影和中文字符显示差异。

inline 模式下，ReAct / Plan / Team 的思考内容默认收起。ReAct 思考期间 `Ctrl+T` 切换实时预览；回到输入行后 `Ctrl+T` 打开/关闭最近思考详情，`Ctrl+O` 查看最近工具/代码块，`PgUp/PgDn` 滚动详情。详情使用 JLine 临时区域，不重复打印正文。最近思考最多保留 20 段，并按约 20 万字符预算淘汰旧记录；新任务不立即丢弃上一轮思考。运行时提前键入的普通文字保留到下一次输入框，需要再次 Enter 确认，不自动执行。
...
| 形态 | 启用方式 | 视觉风格 |
|---|---|---|
| **inline 流式 TUI**（默认） | 直接运行 / `PAICLI_RENDERER=inline` | Claude Code / Qoder 风格：π 主题彩色开屏、主屏直出、transcript 当前位置的 `* ` 输入提示、JLine `Status` 托管的底部 dock（YOLO/HITL、MCP、Skill、model、ctx、token、cwd 等关键字段带克制彩色高亮；ctx 是当前上下文估算，in/out/cache 是调用统计）、JLine 原生清理已提交输入行、右侧输入提示、行内可折叠工具块（`Read 3 files (ctrl+o to expand)`）、行内 git diff、HITL 单字符 `[y/n/a/s/m]` 提示 |
...
║ ╚══════╝╚═╝╚═╝     ╚═╝╚═╝     ╚══════╝╚══════╝ ╚═════╝╚══════╝╚═╝  ║
║                                                                    ║
║         SIMPLECLI  /  Terminal-First Agent CLI  /  v16.1.0         ║
║                                                                    ║
╚════════════════════════════════════════════════════════════════════╝
